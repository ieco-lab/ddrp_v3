...
dd_stress_results <- foreach(dat = dats_list, .packages = pkgs, 
  ...
  foreach(d = unname(unlist(dat)), .packages = pkgs, .inorder = TRUE) %dopar% {
      ...
      # Get position (layer) of date in multi-layerd raster
      lyr <- which(dats2 == d)
      
      # Make the plots
      PlotMap(subset(rast("DDtotal.tif"), lyr), d, 
      ...
      if (exclusions_stressunits) {
        ...
        PlotMap_stress(subset(rast("Cold_Stress_Units.tif"), lyr), d, 
        ...
        PlotMap(subset(rast("Cold_Stress_Excl.tif"), lyr), d, 
                "Cold stress exclusion", "Exclusion status", 
                "Misc_output/Cold_Stress_Excl")

        # Heat unit accumulation
        PlotMap_stress(subset(rast("Heat_Stress_Units.tif"), lyr), d, 
...
# Make list of Stage Count rasters for plotting
if (exclusions_stressunits) {
...
for (i in 1:length(StageCt_lst)) {
  ...
  foreach(dat = dats_list, .packages = pkgs, .inorder = TRUE) %:%
    foreach(d = unname(unlist(dat)), 
      ...
      # Get position (layer) of date in raster rast, subset layer from rasters,
      ...
      StageCt_df <- ConvDF(rast(StageCt_lst[i])[[lyr]]) %>% 
...
if (pems) {
  ...
  PEM_event_labels <- cbind(data.frame("pem_types" = PEM_types), 
    ...
    mutate(genLabel = case_when(
      gen == 0 ~ "date of OW gen.",
      gen == 1 ~"date of 1st gen.",
      gen == 2 ~ "date of 2nd gen.",
      gen == 3 ~"date of 3rd gen.",
      gen == 4 ~ "date of 4th gen."
    )) %>% 
    mutate(eventLabel = case_when(
      stg == "e" ~ eggEventLabel, 
      stg == "l" ~ larvaeEventLabel,
      stg == "p" ~ pupaeEventLabel,
      stg == "a" ~ adultEventLabel)) %>%
    mutate(finalLabel = paste(genLabel, eventLabel)) %>% 
                      select(pem_types, finalLabel)
  ...
  foreach(type = PEM_types, .packages = pkgs, .export = "start_year") %dopar% {
  #for (type in PEM_types) {
   #print(type)
    # Find files by type (e.g., "PEMe1" for each cohort) 
    files_by_type <- PEM_files[grep(pattern = type, x = PEM_files, fixed = TRUE)] 
    # Change 0 values to NA so they are not averaged 
    PEM_r <- rast(files_by_type)
    PEM_r[PEM_r == 0] <- NA
    
    # Remove PEM raster files if all PEM values are 0 because 
    # there's no point in plotting an all zero result
    if (sum(matrix(PEM_r), na.rm = TRUE) == 0) {
      unlink(list.files(pattern = glob2rx(paste0("*", type, "_*tif$"))))
    # Remove PEM raster file if there is only a single unique value
    # This may happen for the last day of sampling period? (check on this)
    } else if (length(unique(values(PEM_r))) < 3) {
        vals <- unique(getValues(PEM_r))
        vals <- vals[!is.na(vals)]
        if (length(vals) == 1) {
          unlink(list.files(pattern = glob2rx(paste0("*", type, "_*tif$"))))
        }
      # If PEM raster has data, then process and plot it
    } else {
    # Create event label to be used for making summary maps
      eventLabel_df <- filter(PEM_event_labels, PEM_types == type) %>% 
        select(finalLabel) %>%
        mutate(., finalLabel = ifelse(
          type == OW_pem, paste("date of OW gen.", OWEventLabel), finalLabel))          
        eventLabel <- paste(eventLabel_df$finalLabel)
        
      # Remove layers in the PEM raster if they are all NA, and print warning 
      # If this happens, should maybe change cohort emergence params
      notNA <- global(PEM_r, fun="notNA") # Quantifies # of non-NA cells 
      PEM_r <- PEM_r[[which(notNA != 0)]] # Removes layers with all NA values
      if (nlyr(PEM_r) < ncohort) {
      ...
      # Then save multi-layered raster, and create and save summary maps
      ...
      # doesn't crash.
      avg_PEM <- weighted.mean(PEM_r, relpopsize, na.rm = TRUE) 
...
#for (stg in stage_list) {
  ...
  if (stg != stg_nonOW) {
    ...
    Lfstg_plots <- foreach(lyr = 1:nlyr(Lfstg_wtd), 
        ...
        PlotMap(Lfstg_wtd[[lyr]], lyr_name, paste0(stg_nam, " relative pop. size"), 
    ...
    if (exclusions_stressunits) {
      ...
      # rasters. This is much more memory efficient than weighting
      # the the LifestageEXCL1 and LifestageEXCL2 rasters separately
      Lfstg_wtd_excl1 <- rast(Rast_Subs_Excl(Lfstg_wtd, "Excl1"))
      ...
      Lfstg_wtd_excl2 <- rast(Rast_Subs_Excl(Lfstg_wtd, "Excl2"))
...
# Make a list of cohort rasters for each generation
gen_fls_lst <- list()
...
# Weight the NumGen rasters for each generation according to the relative
...
foreach(i = 1:length(gen_fls_lst), .packages = pkgs, .inorder = TRUE) %dopar% {
#for (i in 1:length(gen_fls_lst)) { 
  ...
  # The value is NA here, becaue the rasters have already been split out by 
  ...
  # All Stress Exclusion raster with the weighted NumGen raster than to weight
  ...
  if (exclusions_stressunits) {
    ...
    # NumGenEXCL2  
    NumGen_wtd_excl2 <- rast(Rast_Subs_Excl(NumGen_wtd, "Excl2"))
...
# together, and then save the raster to a file. This allows the stacked results 
...
if (exclusions_stressunits) {
  ...
  writeRaster(rast(unlist(NumGen_fls, use.names = FALSE)),
              filename = "NumGen_all_merged.tif")
}
...
NumGen_mrgd_fls <- list("NumGen_all_merged.tif")
if (exclusions_stressunits) {
  NumGen_mrgd_fls <- append(
    NumGen_mrgd_fls, 
...
foreach(i = 1:length(NumGen_mrgd_fls), .packages = pkgs,.inorder = TRUE) %:%
  foreach(d = dats_list, .packages = pkgs, .inorder = TRUE) %dopar% {
    ...
    # Get the multi-layered raster for the file type
    gen_fl <- NumGen_mrgd_fls[[i]]
    fl_type <- names(rast(NumGen_mrgd_fls[[i]])[[1]])
    ...
    # Then loop through date vector, extract (subset) the raster for the
    ...
    for (dat in dat_vec) {
      #print(dat)
      # Which layer # in the rasters corresponds to the date? Then subset raster.
      lyr_no <- which(dats2 == dat) 
      lyr_name <- paste0("\\b[.]", lyr_no, "\\b") # exact match
      r_sub <- rast(gen_fl)[[grep(lyr_name, names(rast(gen_fl)))]]
      
      # In order for all completed generations to show up in legend key, need to 
      # extract info from "NumGen_all_merged" multi-layered raster for the date 
      ...
      # non-zero values.
      if (exclusions_stressunits) {
        NumGen_r <- rast(NumGen_mrgd_fls[[1]])
        r_sub2 <- NumGen_r[[grep(lyr_name, names(NumGen_r))]]
        maxgens <- as.data.frame(r_sub2) %>%
         # select(-x, -y) %>% 
      ...
      r_sub_uniqueVals <- unique(
        c(as.matrix(r_sub)))[!is.na(unique(c(as.matrix(r_sub))))]
      
      if (all(r_sub_uniqueVals < 0)) {
        mrgd2 <- df
      }
      
      # Merge data frames in the list - if list is empty b/c all data frames 
      # had value = 0, then just use most recent data frame
      if (any(r_sub_uniqueVals > -1) & length(NumGen_lyrs_toPlot) > 0) {
      ...
      } else if (any(r_sub_uniqueVals > 0) & any(mrgd$value < 0)) {
        if (any(mrgd$value < 0)) {
          excl_vals <- data.frame(mrgd %>% filter(value != 0))
          #excl_vals$gen <- "GenOW"
          if (any(df$gen == 0)) {
...
# List to store results for each date
all_combos_lst <- list()

# Note: there are issues with parallelizing this process in "terra" - avoid.
# The "raster" version of DDRP parallelized parts of this.
for (i in 1:length(dats2)) { 
  
  # Get file and layer names
  NumGen_all_fl <- "NumGen_all_merged.tif"
  all_lyr_names <- names(rast(NumGen_all_fl))
                               
  combos_lst <- list()
  
  for (j in 1:length(gens_combo_all)) {
    
    # Which layer # in raster corresponds to the date? Then subset raster
    # and replace all non-zero values w/ 1, and 0 values with NA
    lyr_no <- which(dats2 == dats2[i]) 
    lyr_name <- paste0("\\b", lyr_no, "$\\b") # regex for getting exact match
    sub1 <- rast(NumGen_all_fl)[[grep(lyr_name, all_lyr_names)]]
    sub1[sub1 > 0] <- 1 
    sub1[sub1 == 0] <- NA
    
    # Get each gen in the combo
    genA <- str_split_fixed(gens_combo_all[j], pattern = "\\|", 2)[,1]
    genB <- str_split_fixed(gens_combo_all[j], pattern = "\\|", 2)[,2]
    
    # Search for each of those gens in the stack and combine them
    # The order matters here, genA must be first layer
    sub1A <- subset(sub1, grep(paste0(genA, "[.]"), names(sub1)))
    sub1B <- subset(sub1, grep(paste0(genB, "[.]"), names(sub1)))
    sub2 <- c(sub1A, sub1B)
    rm(sub1A, sub1B) # Free up memory
    
    # Identify areas where the two gens overlap by summing them
    # Areas of overlap = 2
    gen_ovlp <- app(sub2, fun = sum, na.rm = TRUE) 
    ...
    if (!is.na(any(values(sub2[[2]] > 0)))) {
      gen_ovlp_NA <- lapp(c(sub2[[1]], gen_ovlp),
        fun = function(x, y) {
          x[y == 2] <- NA
    ...
    # Need to add the very first layer back in (Gen0)
    if (j == length(gens_combo_all)) {
      gen0 <- sub1[[1]]
      gen_ovlp_NA <- c(gen0, gen_ovlp_NA)
    }
    
    # "Corrected" combo is put back into a list
    combos_lst[[j]] <- gen_ovlp_NA 
  }
    
  # Create a raster from the list
  all_combos_lst[[i]] <- rast(combos_lst)
}

# Combine results of the previous analysis and overwrite the 
# "NumGen_all_merged.tif" file. This raster file has the "corrected" layers.
...
# Now use resulting ordered NumGen raster layers to mask out Lifestage 
...
if (exclusions_stressunits) {
  cat("\n", str_wrap("Assigning a generation number to pixels in the Adult, 
                   Adult_Excl1, and Adult_Excl2 rasters", width = 80), 
      sep = "", file = Model_rlogging, append = TRUE)
  cat("\n", str_wrap("Assigning a generation number to pixels in the Adult, 
                   Adult_Excl1, and Adult_Excl2 rasters", width = 80),
      sep = "")
...
#for (gen in 0:maxgens) {
    ...
    NumGen_msk <- subset(
      rast("NumGen_all_merged.tif"),
      grep(paste0("Gen_", gen, "[.]"), names(rast("NumGen_all_merged.tif"))))
...
# TO DO: maybe change way the results are displayed; a bit confusing to have 
# "other stages" plus gen number, but gen number is not actually displayed on map.
RegCluster(round(ncores/6))
...
#for (j in 1:length(Adult_byGen_fls)) {
 fl_type <- paste0(names(Adult_byGen_fls[j]))
   #for (d in dats2) {
    ...
    # Which layer # in the raster corresponds to the date? 
    lyr_no <- which(dats2 == d) 
    lyr_name <- paste0("\\b", lyr_no, "$\\b") # regex for getting exact match
    
    # Subset the raster by the layer name (date)
    ...
    for (f in 1:length(fls)) {
      r <- rast(fls[[f]])[[grep(lyr_name, names(rast(fls[[f]])))]]
      ...
      if (any(!is.na(values(r)))) { # Don't include layers if all values are NA
        freq <- data.frame(freq(r)) # Get frequency of all vals
    ...
    # For each raster layer, identify the generation, convert data to a 
    # data frame and add it to a list
    df_list <- list()
    for (i in 1:nlyr(r_sub)) {
      r <- r_sub[[i]] # raster for each date
      gen <- str_extract(names(r), "[0-9]+") # extract generation from name
      #print(gen)
...
if (start_year == current_year) {
  today_dat_fls <- list.files(
    pattern = glob2rx(paste0("*StageCount*", today_dat, "*.png$")))
